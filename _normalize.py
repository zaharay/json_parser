# ---------------------------------------------------------------------
# Код адаптирован для Python 3.5.3
# ---------------------------------------------------------------------
...
def _json_normalize(
    ...
    def _pull_records(js: Dict[str, Any], spec: Union[List, str]) -> List:
        ...
        if not isinstance(result, list):
            if pd.isnull(result):
                ...
                raise TypeError(
                    "{0} has non list value {1} for path {2}. Must be list or null.".format(js, result, spec)                    
                )
        return result
    ...
    if record_prefix is not None:
        result = result.rename(columns=lambda x: "{0}{1}".format(record_prefix, x))
    ...
    for k, v in meta_vals.items():
        ...
        if k in result:
            raise ValueError(
                "Conflicting metadata name {}, need distinguishing prefix ".format(k)
            )
